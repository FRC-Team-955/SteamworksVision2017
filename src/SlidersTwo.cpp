...
Sliders::Sliders(bool showUI, char* window_title, map<string, int> *sliders, Saving* save_object) {
	this->window_title = window_title;
	this->sliders = sliders;
	this->save_object = save_object;

	// When the constructor is called, the max values of the sliders are passed in. Later the map is used as a table of values to use and live update to, so we must copy it
...
void Sliders::UpdateSliders () { //Write over the sliders - opencv's sliders' value inputs are not marked volatile, so we can't change them manually. 
	for (auto& slider_entry : *sliders) {
		cvCreateTrackbar2(slider_entry.first.c_str(), window_title, &slider_entry.second, slider_limits[slider_entry.first], on_trackbar, this);  
	}
}
